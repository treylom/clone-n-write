...
**저자 A 문체 변환 — 89.3점, 실글 상위 21% (문장 단위 사고 전개 · 절제된 격식 · 독자에게 질문)**
...
**저자 B 문체 변환 — 89.5점, 실글 상위 19% (초단문 행갈이 · 후킹 첫줄 · 리스트 · 붙여쓰기 이완)**
...
같은 내용이 완전히 다른 두 글이 됐습니다 — 문장 호흡(저자 A 문장당 7.8어절 vs 저자 B 3.9어절), 종결 습관(절제된 격식 혼합 vs 음슴·구종결 69%), 독자를 대하는 방식(질문을 건넴 vs 리스트로 후킹)까지. 이 차이는 전부 각 저자의 코퍼스에서 **측정된 대역**이고, 변환은 그 대역에 수렴할 때까지 반려·코칭을 반복한 결과입니다.
...
이 코칭대로 고친 v2가 위의 89.3점입니다. "감으로 비슷하게"가 아니라 — 측정이 반려하고, 진단이 고칠 방향을 말하고, 재채점이 수렴을 확인합니다.
