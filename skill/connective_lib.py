...
설계 결함 (b) 복원·정본화 (설계 히스토리):
  ...
  유실됐던 fix다(설계 결함 b). 본 lib이 그 어휘를 **코드 정본**으로 못박고,
...
근거(정본):
  - 발행 시리즈의 이음말 구조(예고 질문·수미상관·다음편 훅) — 본 파일 예문은 전부 합성.
    the initial phase the gold-vs-draft diff(a prior gate-PASS version ↔ 발행본)에서 the author이 *추가*한 이음이 여기 해당.
...
CONNECTIVES = {
    ...
    "closing_move": {
        "description": "마무리 수 — 후속 약속·겸손·유머 점프·커뮤니티 감사(the qualitative analysis 귀납).",
        "patterns": [
            re.compile(r"(다시 (포스팅|올리|쓰겠|공유)|후속|아직 다듬|계속 고쳐|고쳐 나가는 중|정답은 없|"
